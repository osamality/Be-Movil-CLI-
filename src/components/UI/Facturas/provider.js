...
const renderItems = (ActiveType , ActiveProvider)=>{
  ...
  const navigation = useNavigation()
    ...
    const handelClick = async(index,data)=>{
      if(data.name=="Pago de Facturas"){

        setIndex(index)
        dispatch(FacturasActions.setActiveProvider(data))
        dispatch(FacturasActions.resetInintalValues())
        navigation.navigate("pego")
...
const mapStateToProps = ({Facturas}) => ({
